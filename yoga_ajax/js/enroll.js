...
function butFun () {
	var bac_obj = document.getElementById('content');
	button.value = '提交中...';
	var but_xml = requestObject();
	if (but_xml == null) {
		alert("Unable to create request");
		return;
	}

	var url = 'enrolled.html';
	var xml_data = 'firstname=' + document.getElementById('firstname').value
	...
	//配置请求，post方式
	but_xml.open('POST',url,true);
	//设置回调函数
	but_xml.onreadystatechange = function() {
		if(but_xml.readyState==4 && but_xml.status==200) {

			var t_xml = but_xml.responseText;
			console.log(t_xml);
			bac_obj.innerHTML = t_xml;
		}
	}
	//发送请求
	but_xml.send( xml_data );
}